...
class FrozenParamWithEandC(ContinualModel):
    ...
    def get_parser(parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument('--num_of_observe_before_train_in_replaybuffer',type=int,default=5,
    ...
    def froze_param(self):
        ...
        with torch.no_grad():
            ...
            param_vector =torch.cat([param.view(-1) for _,param in delta_param.items()])
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None) -> float:
        ...
        # 使用回放数据进行微调
        if self.epoch_iteration!=0 and self.epoch_iteration % self.args.num_of_observe_before_train_in_replaybuffer == 0:
            if not self.buffer.is_empty():
                ...
                self.froze_param()
                # log:检查还有多少可被训练的参数
                froze_vector = torch.cat([mask.view(-1)for _,mask in self.freeze_masks.items()])
